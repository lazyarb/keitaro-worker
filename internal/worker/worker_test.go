...
func TestRecoverMovesInterruptedEventBackToPending(t *testing.T) {
	config, _ := testRuntimeConfig(t, "https://app.lazyarb.com/postback/token")
	writeTestEvent(t, config.QueueRoot, "processing", "event.json", Event{
		LegacyEndpoint: "https://app.lazyarb.com/postback/token", Code: "click_install", AdID: "42", SubID: "sub-1",
	})
